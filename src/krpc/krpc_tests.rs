...
use super::{message::kmsg::response, *};
#[cfg(test)]
use crate::dht_node;
use crate::u160::U160;
use tokio;
...
async fn ping() {
    ...
    let client_node = dht_node::DhtNode {
        id: client_id,
        addr: SocketAddr::from_str(client_addr).unwrap(),
    };
    let client = KrpcService::new(client_node, 100, true).await.unwrap();

    let server_addr = "127.0.0.1:12345";
    let server_id = U160::rand();
    let server_node = dht_node::DhtNode {
        id: server_id,
        addr: SocketAddr::from_str(server_addr).unwrap(),
    };
    let server = KrpcService::new(server_node, 100, false).await.unwrap();

    let response = client
        .query(QueryMethod::Ping, server_node.addr)
        .await
        .unwrap();
    assert!(if let Message::Response(Response {
...
async fn timeout_readonly() {
    ...
    let ro_server = KrpcService::new(server_node, 100, true).await.unwrap();

    let response = client
        .query(QueryMethod::Ping, server_node.addr)
        .await
        .unwrap();
    assert!(if let Message::Error(message::Error {
        code, description, ..
    }) = &response
    {
